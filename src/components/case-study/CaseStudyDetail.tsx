...
export function CaseStudyDetail({ caseStudy }: CaseStudyDetailProps) {
  ...
  // Listen to the CONTAINER scroll, not window scroll
  // (window scroll is frozen when dialog is open!)
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-background text-text-primary overflow-y-auto h-screen">
      {/* Scroll Progress Bar */}
      <motion.div
      ...
      <motion.section
        className="w-full h-screen flex items-center justify-center relative z-10"
        style={{
